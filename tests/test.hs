...
testEqual :: Assertion
testEqual = do
    let xs = [0,1,2,3,4,5,67,0,0,2,40,0,20,0,0]
        m1 = MG.fromList (3,5) xs :: MD.Matrix U.Vector Int
        al = filter ((/=0) . snd) $ MD.toList $ MD.imap (\i v -> (i,v)) m1
        m2 = MG.fromList (3,5) xs :: MS.CSR U.Vector Int
        m3 = MS.fromAscAL (3,5) (length al) al :: MS.CSR U.Vector Int
